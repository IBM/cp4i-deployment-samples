...
function run_continuous_load_script() {
  ...
  if [[ "$APIC_ENABLED" == "true" ]]; then
    if ! $CURRENT_DIR/continuous-load.sh -n "$CONTINUOUS_LOAD_NAMESPACE" -b "$DDD_TYPE" -a -z 100 -t 0 -s ; then
      echo -e "$CROSS [ERROR] Could not start or finish the continuous load testing with apic enabled."
      divider
      exit 1
    fi
  else
    if ! $CURRENT_DIR/continuous-load.sh -n "$CONTINUOUS_LOAD_NAMESPACE" -b "$DDD_TYPE" -z 100 -t 0 -s ; then
