...
EOF
  ...
  while [ $numberOfMatchesForImageTag -ne $numberOfReplicas ]; do
    ...
    if [[ $numberOfMatchesForImageTag != "$numberOfReplicas" ]]; then
      echo -e "INFO: Not all $release_name pods have been deployed with the new image having the image tag '$imageTag', retrying for upto 10 minutes for new $release_name demo pods te be deployed with new image. Waited ${time} minute(s)."
      sleep 60
    else
